...
from .models import SessionBlock, UsageEntry, normalize_model_name
...
class UsageAggregator:
    ...
    async def aggregate_from_blocks(
        ...
        if view_type == "daily":
            return await self.aggregate_daily(all_entries)
        return await self.aggregate_monthly(all_entries)
    ...
    async def aggregate(self) -> list[dict[str, Any]]:
        ...
        """
        from .reader import load_usage_entries

        logger.info(f"Starting aggregation in {self.aggregation_mode} mode")

        # Load usage entries
        entries, _ = await load_usage_entries(data_path=self.data_path)
        ...
        if self.aggregation_mode == "daily":
            return await self.aggregate_daily(entries)
        if self.aggregation_mode == "monthly":
            return await self.aggregate_monthly(entries)
